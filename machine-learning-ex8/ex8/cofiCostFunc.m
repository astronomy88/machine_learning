...
% My hint - just play with X, Theta, Y and R in Part 2 to get the dimensions right
...
for i=1:num_movies
	idx = find(R(i, :) == 1); %-- list of all users that rated movie i
	Y_tmp = Y(i, idx);
	Theta_tmp = Theta(idx, :);
...
for j=1:num_users
	idx = find(R(:, j) == 1); %-- index of all movies that user j rated
	Y_tmp = Y(idx, j);
	X_tmp = X(idx, :); %-- movies that have reviews
